...
<CustomCheckbox checked={value} onChange={handleChange} label="Option" />;
```
...
<EmojiToggle checked={enabled} onChange={toggle} label="Enable Feature" />;
```
...
// Old
<div>
  <input type="checkbox" id="option1" checked={checked} onChange={handleChange} />
  <label htmlFor="option1">Option Label</label>
</div>;

// New
import { CustomCheckbox } from "@/components/common";
<CustomCheckbox checked={checked} onChange={handleChange} label="Option Label" />;
```
...
// New
import { AnimatedInput } from "@/components/common";
<AnimatedInput value={value} onChange={onChange} placeholder="Enter text" label="Field Label" />;
```
...
<div className="space-y-4">
  ...
  <CustomCheckbox checked={agree} onChange={(e) => setAgree(e.target.checked)} label="I agree" />
