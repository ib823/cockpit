...
describe.skipIf(skipTests)("SQL Injection Prevention", () => {
  ...
  test("Prisma prevents UNION-based SQL injection", async () => {
    ...
    await expect(prisma.projects.findFirst({ where: { id: maliciousId } })).resolves.not.toThrow();
  ...
  test("Prisma prevents comment-based SQL injection", async () => {
    const maliciousInputs = ["admin'--", "'; DROP TABLE users; --", "1'; DELETE FROM projects; --"];

    for (const input of maliciousInputs) {
      await expect(prisma.projects.findFirst({ where: { id: input } })).resolves.not.toThrow();
  ...
  test("Email input sanitization prevents injection in users table", async () => {
    ...
    for (const email of maliciousEmails) {
      await expect(prisma.users.findUnique({ where: { email } })).resolves.not.toThrow();
