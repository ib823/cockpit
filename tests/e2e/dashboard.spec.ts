...
test.describe('Comprehensive Dashboard v2', () => {
  ...
  test('should display validation status', async ({ page }) => {
    ...
    const footer = page.locator('[aria-label*="validation"]').or(
      page.locator('text=/validation|All Validations Passed|Critical Issue/i')
    );
  ...
  test('should show AI recommendations', async ({ page }) => {
    ...
    const recommendations = page.locator('[aria-label*="recommendation"]').or(
      page.getByRole('listitem').filter({ hasText: /Recommendation|Optimize|Improve/ })
    );
...
test.describe('Dashboard Export Functionality', () => {
  test('should export to PDF', async ({ page, context }) => {
    ...
    await page.getByRole('button', { name: /Export|OK/i }).last().click();
