...
test.describe('New Project Creation Journey', () => {
  test('user can create project with phases and tasks', async ({ page }) => {
    ...
    await page.fill('input[name="name"], input[placeholder*="name" i]', 'E2E Test Project - New Creation');
    await page.fill('input[name="description"], textarea[name="description"]', 'Automated test project created via Playwright E2E test');
    ...
    await expect(page.locator('h1, [data-testid="project-title"]')).toContainText('E2E Test Project');
    ...
    await page.fill('input[name="phaseName"], input[placeholder*="phase" i]', 'Planning & Analysis');
    ...
    await page.fill('input[name="taskName"], input[placeholder*="task" i]', 'Requirements Gathering');
