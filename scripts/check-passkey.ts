  ...
  try {
    ...
    if (user.webauthnChallenges.length === 0) {
      ...
      user.webauthnChallenges.slice(0, 3).forEach((ch, i) => {
        ...
        console.log(`     • Expires: ${ch.expiresAt.toISOString()} ${expired ? '⚠️ EXPIRED' : '✓ Valid'}`);
        console.log('');
      });
    }
    ...
    if (adminApproval) {
      ...
      if (adminApproval.expiresAt) {
        ...
        console.log('    • Expires:', adminApproval.expiresAt.toISOString(), expired ? '⚠️ EXPIRED' : '✓ Valid');
        if (expired) {
    ...
    } else if (adminApproval && adminApproval.expiresAt && new Date(adminApproval.expiresAt) < new Date()) {
