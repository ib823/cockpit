...
async function checkWebAuthnConfig() {
  try {
    ...
    console.log('  • WEBAUTHN_ORIGIN:', process.env.WEBAUTHN_ORIGIN || 'NOT SET (using http://localhost:3001)');
    ...
    if (isProduction) {
      if (!process.env.WEBAUTHN_RP_ID || !process.env.WEBAUTHN_ORIGIN) {
        ...
        console.log('  2. Set WEBAUTHN_ORIGIN to your full URL (e.g., "https://app.yourdomain.com")');
