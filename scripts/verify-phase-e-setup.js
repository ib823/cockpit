...
checkDirectory("public/screenshots", "Screenshots", [
  "desktop-dashboard.png",
  "mobile-estimator.png",
]);
...
if (checks.warnings.length > 0 || checks.failed.length > 0) {
  ...
  if (
    checks.warnings.some(
      (w) => w.includes("Hotjar") || w.includes("Mixpanel") || w.includes("Analytics")
    )
  ) {
  ...
  if (
    checks.warnings.some((w) => w.includes("Icons")) ||
    checks.warnings.some((w) => w.includes("Screenshots"))
  ) {
