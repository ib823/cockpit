...
function generatePriorityList(components: ComponentData[]): string {
  ...
  const criticalA11y = components.filter(
    (c) => !c.hasA11yAttrs && c.performanceNotes.includes("Large component")
  );
  ...
  criticalA11y
    ...
    .forEach((c, i) => {
      ...
      output += `   - Issues: ${c.performanceNotes
        .split(";")
        .map((s) => s.trim())
        .join(", ")}\n`;
  ...
  const highPerf = components.filter(
    (c) =>
  ...
  // Medium: Missing A11y
  const mediumA11y = components.filter(
    (c) => !c.hasA11yAttrs && !c.performanceNotes.includes("Large component") && c.hooks !== "none" // Interactive components
  );

  output += "\n## 🟡 Medium Priority: Accessibility Only\n\n";
  output += `${mediumA11y.length} interactive components need accessibility attributes\n\n`;

  mediumA11y.slice(0, 30).forEach((c, i) => {
...
function generateHookPatterns(components: ComponentData[]): string {
  ...
  // Hook combinations
  const useState_useEffect = components.filter(
    (c) => c.hooks.includes("useState") && c.hooks.includes("useEffect")
  );
  const useState_useCallback = components.filter(
    (c) => c.hooks.includes("useState") && c.hooks.includes("useCallback")
  );
  const useMemo_useCallback = components.filter(
    (c) => c.hooks.includes("useMemo") && c.hooks.includes("useCallback")
  );
...
async function main() {
  ...
  fs.writeFileSync(path.join(auditDir, "refactoring-priorities.md"), priorityList);

  console.log("Generating dependency report...");
  const depReport = generateDependencyReport(components);
  fs.writeFileSync(path.join(auditDir, "dependency-analysis.md"), depReport);

  console.log("Generating hook patterns...");
  const hookPatterns = generateHookPatterns(components);
  fs.writeFileSync(path.join(auditDir, "hook-patterns.md"), hookPatterns);

  console.log("Generating accessibility report...");
  const a11yReport = generateAccessibilityReport(components);
  fs.writeFileSync(path.join(auditDir, "accessibility-remediation.md"), a11yReport);
