...
export const POST = withCsrfProtection(async (req: Request) => {
  try {
    ...
    if (!data.name || !data.phases) {
      return Response.json({ error: "Missing required fields: name, phases" }, { status: 400 });
    }
    ...
    data.phases.forEach((phase: Phase) => {
      csvRows.push([
        ...
        `${(((phase.effortMD || 0) / totalMD) * 100).toFixed(1)}%`,
      ]);
    });
    ...
    if (data.resources && data.resources.length > 0) {
      ...
      // Resource totals
      const totalFTE = data.resources.reduce((sum: number, r: Resource) => sum + (r.fte || 0), 0);
      const totalCost = data.resources.reduce((sum: number, r: Resource) => sum + (r.totalCost || 0), 0);
      csvRows.push(["TOTAL", totalFTE.toFixed(1), "", `$${totalCost.toLocaleString()}`]);
    }
