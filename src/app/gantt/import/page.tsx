...
export default function GanttImportPage() {
  ...
  const parseB = () => {
    try {
      ...
      const map = { role: 'Role', rank: 'Rank', company: 'Company', resourceName: 'Resource Name', start: 'Start Date', end: 'End Date', total: 'Total' };
    ...
    <div style={{ padding: 40, maxWidth: 900, margin: '0 auto' }}>
      ...
      {step === 'A' && (
        <div>
          ...
          <p>
            Paste TSV or CSV with columns: <code>Task Name</code>, <code>Start</code>, <code>End</code>, <code>Duration</code>, and <code>W1, W2, W3...</code> (flag with X or 1).
          </p>
          <textarea
            value={rawTasks}
            onChange={(e) => setRawTasks(e.target.value)}
            rows={12}
            style={{ width: '100%', fontFamily: 'monospace', fontSize: 13, padding: 8, border: '1px solid #ccc', borderRadius: 4 }}
      ...
      {step === 'B' && (
        <div>
          ...
          <p>
            Paste TSV or CSV with columns: <code>Role</code>, <code>Rank</code>, <code>Company</code>, <code>Resource Name</code>, <code>Start Date</code>, <code>End Date</code>, <code>Total</code>, and <code>W1, W2, W3...</code> (mandays).
          </p>
          <textarea
            value={rawResources}
