...
export function parseResourceData(
  ...
  if (!tsvText || !tsvText.trim()) {
    return {
      success: false,
      errors: [
        { row: 0, column: "general", message: "No resource data provided", severity: "error" },
      ],
      warnings: [],
    };
  }
      ...
      errors: [
        {
          row: 1,
          column: "general",
          message: "No weekly columns found. Expected: W1, W2, ... or date headers.",
          severity: "error",
        },
      ],
      warnings: [],
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    for (let w = 0; w < effortCells.length && w < weekDates.length; w++) {
      ...
      if (days > MAX_DAYS_PER_WEEK) {
        warnings.push(
          `Row ${i + 1}, Week ${w + 1}: ${days} days exceeds standard workweek (${MAX_DAYS_PER_WEEK} days)`
        );
      }
    ...
    if (!designation) {
      ...
      warnings.push(
        `Row ${i + 1}: Designation "${sanitizedDesignation}" not recognized. Needs manual mapping.`
      );
      continue;
    }
  ...
  if (resources.length === 0 && unmappedResources.length === 0) {
    return {
      success: false,
      errors: [
        { row: 0, column: "general", message: "No valid resources found", severity: "error" },
      ],
      warnings: [],
    };
  }
