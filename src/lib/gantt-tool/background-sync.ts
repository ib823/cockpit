  ...
  if (delta.projectUpdates) {
    ...
    Object.keys(delta.projectUpdates).forEach((key) => {
      ...
      if (key === 'startDate' && value) {
        serialized.projectUpdates![key] = formatDateField(value);
      } else {
        serialized.projectUpdates![key] = value;
      }
    });
  }
    ...
    for (const item of pendingItems) {
      try {
        ...
        if (item.retryCount >= MAX_RETRY_COUNT) {
          ...
          console.log(`[BackgroundSync] Retrying in ${delay}ms (attempt ${item.retryCount + 1}/${MAX_RETRY_COUNT})`);
