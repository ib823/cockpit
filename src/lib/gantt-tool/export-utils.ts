...
function addPDFFooter(pdf: jsPDF, project: GanttProject): void {
  ...
  for (let i = 2; i <= pageCount; i++) {
    ...
    // Page number (right)
    pdf.text(`Page ${i - 1} of ${pageCount - 1}`, pageWidth - 20, pageHeight - 8, {
      align: "right",
    });
...
async function prepareExportCanvas(
  ...
  const targetWidth =
    config.sizePreset === "custom" && config.customWidth ? config.customWidth : sizePreset.width;
  const targetHeight =
    config.sizePreset === "custom" && config.customHeight ? config.customHeight : sizePreset.height;
