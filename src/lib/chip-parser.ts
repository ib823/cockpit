...
const CHIP_PATTERNS = {
  // Geographic patterns
  country: [
    /\b(malaysia|singapore|vietnam|thailand|indonesia|philippines)\b/gi,
  ],
  
  // Employee patterns only (not users)
  employees: [
    /(\d+(?:,\d{3})*)\s*(?:employees?|staff|headcount)\b/gi,
  ],
  ...
  compliance: [
    /\b(?:e-?invoice|einvoice|my\s*invois)\b/gi,
    /\b(?:LHDN|SST|GST)\b/gi,
  ],
...
function parseValue(kind: ChipKind, raw: string, match: RegExpMatchArray) {
  switch (kind) {
    ...
    case 'modules': {
      ...
      if (normalized.includes('supply') || normalized.includes('scm')) return { value: 'Supply Chain' };
      return { value: raw };
    }
    ...
    case 'industry': {
      ...
      if (industryMatch) {
        return { value: industryMatch[1].charAt(0).toUpperCase() + industryMatch[1].slice(1).toLowerCase() };
      }
      return { value: raw };
    }
