...
export function validateChipCompleteness(chips: Chip[]): CompletenessResult {
  ...
  const requiredScore = REQUIRED_CHIP_TYPES.filter(type => chipTypes.has(type)).length / REQUIRED_CHIP_TYPES.length * 60;
  const recommendedScore = RECOMMENDED_CHIP_TYPES.filter(type => chipTypes.has(type)).length / RECOMMENDED_CHIP_TYPES.length * 30;
  const qualityScore = chips.length > 0 ? Math.min(10, chips.length / 5 * 10) : 0;
  const confidenceScore = chips.length > 0 ? (chips.reduce((sum, c) => sum + c.confidence, 0) / chips.length) * 10 : 0;

  const score = Math.round(requiredScore + recommendedScore + qualityScore - (lowConfidenceChips.length * 2));
...
export function validatePhaseSequencing(phases: Phase[]): SequencingResult {
  ...
  if (totalDays > 365) {
    suggestions.push(`Total project duration is ${totalDays} days (>1 year). Consider parallel execution or scope reduction.`);
  }

  const totalEffort = phases.reduce((sum, p) => sum + (p.effort || 0), 0);
  if (totalEffort > 5000) {
    suggestions.push(`Total effort is ${totalEffort} person-days. This is a large project - ensure adequate resources.`);
  }
