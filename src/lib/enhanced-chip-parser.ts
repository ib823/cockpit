...
import { CRITICAL_PATTERNS, extractNumericValue, EFFORT_IMPACT_RULES, MISSING_INFO_ALERTS } from '@/lib/critical-patterns';
...
export function parseRFPTextEnhanced(text: string): ExtendedChip[] {
  ...
  for (const critical of criticalChips) {
    const isDuplicate = allChips.some(existing => 
      existing.type === critical.type && 
      String(existing.value) === String(critical.value)
    );
...
function extractCriticalPatterns(text: string): ExtendedChip[] {
  ...
  const vagueEmployeePattern = /(?:few|several|many)\s+(hundred|thousand)\s+(?:people|employees|staff)/gi;
