...
export async function generatePowerPoint(data: ProjectData): Promise<Buffer> {
  ...
  slide1.addText(data.name, {
    ...
    fontSize: 24, color: 'E6F7FF', align: 'center'
  });
  ...
  const summaryData = [
    ...
    [
      { text: 'Total Effort' },
      { text: `${data.totalMD.toFixed(0)} MD` }
    ],
    [
      { text: 'Project Duration' },
      { text: `${data.durationMonths.toFixed(1)} months` }
    ],
    [
      { text: 'PMO Effort' },
      { text: `${data.pmoMD.toFixed(0)} MD` }
    ],
    [
      { text: 'Estimated Cost' },
      { text: `$${(data.totalMD * 150 * 22).toLocaleString()}` }
    ],
  ];
  ...
  if (data.resources && data.resources.length > 0) {
    ...
    const resourceData = [
      ...
      [
        ...
        { text: data.resources.reduce((sum, r) => sum + r.fte, 0).toFixed(1), options: { bold: true } },
        { text: '' },
        {
  ...
  nextSteps.forEach(step => {
    slide7.addText(step, {
      ...
      fontSize: 16, color: colors.text, bullet: true
    });
    yPos += 0.6;
  });
