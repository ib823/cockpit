...
function parseDate(s?: string): string | undefined {
  ...
  if (isNumber(t)) {
    const ms = EXCEL_BASE_MS + Number(t) * DAY_MS;
  ...
  if (m) {
    ...
    return toIso(new Date(Date.UTC(y, +mm - 1, +dd)));
  }
  const d = new Date(t);
  return isNaN(+d) ? undefined : toIso(d);
}
...
export function parseTasksSection(tbl: ParsedTable) {
  ...
  tbl.rows.forEach((r, i) => {
    const name = (r[nameIx] || "").trim();
    if (!name) return;
    const start = parseDate(r[sIx]);
    const end = parseDate(r[eIx]);
    ...
    weekCols.forEach(({ i: ix, m }) => {
      ...
      const active =
        token === "X" || token === "x" || token === "1" || token.toLowerCase() === "true";
      if (active) {
        const w = Number(m![1]);
        (flagged[w] ||= []).push(id);
      }
  ...
  weeks.forEach(({ w, startISO, endISO }) => {
    if (flagged[w]?.length) {
      activeByWeek[w] = flagged[w][0];
      return;
    }
    // fallback by date overlap
    const pick = tasks.find((t) => {
      const s = t.start || startISO;
      const e = t.end || endISO;
...
export type ResourceMap = {
  ...
  total?: string; // header names
};

export function parseResourcesSection(
  tbl: ParsedTable,
  map: ResourceMap,
  weeks: WeekBucket[],
  ...
  tbl.rows.forEach((r) => {
    ...
    const resourceName =
      (get(r, "Resource Name") || get(r, map.resourceName) || "").trim() || undefined;
    ...
    weekCols.forEach(({ i: cix, m }) => {
      ...
      const w = Number(m![1]);
      const wk = weeks.find((x) => x.w === w);
      if (!wk) return;

      if (filterByDates && (startISO || endISO)) {
        const s = startISO || wk.startISO;
        const e = endISO || wk.endISO;
