...
import { CheckCircle, TrendingUp, TrendingDown, Clock, DollarSign, AlertTriangle, ArrowRight } from 'lucide-react';
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {decision.options.map((option) => {
                    ...
                    const isHovering = previewDecision === decision.id && previewOption?.id === option.id;
                        ...
                        {option.impact && (isSelected || isHovering) && (
                          <div className="mt-3 pt-3 border-t border-gray-200 flex flex-wrap gap-2 text-xs">
                            {option.impact.duration !== 0 && (
                              <span className={cn(
                                'px-2 py-1 rounded',
                                option.impact.duration > 0 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                              )}>
                                {option.impact.duration > 0 ? '+' : ''}{option.impact.duration}w
                              </span>
                            )}
                            {option.impact.cost !== 0 && (
                              <span className={cn(
                                'px-2 py-1 rounded',
                                option.impact.cost > 0 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                              )}>
                                {option.impact.cost > 0 ? '+' : ''}{formatCurrency(option.impact.cost, 'MYR')}
                              </span>
                            )}
                          </div>
                        )}
              ...
              <div className="space-y-4">
                <div className="p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="text-2xl font-light">
                    {previewOption.impact.duration > 0 ? '+' : ''}
                    {previewOption.impact.duration} weeks
                  </div>
                </div>
                ...
                <div className="p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="text-2xl font-light">
                    {previewOption.impact.cost > 0 ? '+' : ''}
                    {formatCurrency(previewOption.impact.cost, 'MYR')}
                  </div>
                </div>
