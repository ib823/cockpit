  ...
  const filteredModules = useMemo(() => {
    ...
    if (selectedCategory !== "all") {
      const categoryModuleIds =
        PACKAGE_CATEGORIES[selectedCategory as keyof typeof PACKAGE_CATEGORIES] || [];
        ...
        {requiredDependencies.some((dep) => !selectedPackages.includes(dep)) && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
            <p className="text-sm text-yellow-800">
              Missing required dependencies. Enable auto-select or add manually.
            </p>
          </div>
        )}
      </div>
          ...
          return (
            ...
            >
              ...
              {isRequired && !isSelected && (
                <div className="mt-2 text-xs text-orange-600">Required dependency</div>
              )}
            </div>
          );
        })}
