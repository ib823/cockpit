...
export function RateCardManager({ isOpen, onClose, onSave }: RateCardManagerProps) {
  ...
  const [rates, setRates] = useState<Record<ResourceDesignation, number>>(
    Object.entries(DEFAULT_RATE_CARD).reduce(
      (acc, [key, value]) => {
        ...
        >
          <Space direction="vertical" size={4}>
            ...
            <Text style={{ color: "#0050B3" }} className="text-sm">
              ...
              Ringgit (MYR). Rates are based on typical SAP consulting market rates in Malaysia.
            </Text>
          </Space>
        </div>
          ...
          <Space direction="vertical" size={8} style={{ width: "100%" }}>
            {hypotheticalTeam.map((member) => (
              ...
              >
                ...
                <Text strong>{formatMYR(rates[member.designation] * 20 * member.count)}</Text>
              </div>
            ))}
