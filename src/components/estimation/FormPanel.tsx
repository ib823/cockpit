...
export function FormPanel({ projectId, forms, onChange, readonly = false }: FormPanelProps) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'type',
      render: (type: string) => (
        <Tag color={formTypeColors[type]}>{formTypeLabels[type]}</Tag>
      ),
    },
    {
      title: 'Languages',
  ...
  return (
    <ResponsiveCard padding="lg">
      ...
      <div style={{ display: 'flex', gap: '24px', marginBottom: '24px' }}>
        <div>
          <Text size="sm" color="muted">Total Forms</Text>
          <Text size="xl" weight="bold">{forms.length}</Text>
        </div>
        <div>
        ...
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          ...
          <div>
            ...
            <Select
              ...
              onChange={complexity => setFormData({ ...formData, complexity: complexity as Complexity })}
            >
