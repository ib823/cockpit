    ...
    <Space direction="vertical" style={{ width: '100%' }} size="middle">
      ...
      <Card title="🎯 Estimate Summary">
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded">
          <Text type="secondary" className="block mb-2">Complexity Coefficients:</Text>
          <Space split={<span className="text-gray-300">|</span>}>
            <Text>Sb: <Text strong>{results.coefficients.Sb.toFixed(3)}</Text></Text>
            <Text>Pc: <Text strong>{results.coefficients.Pc.toFixed(3)}</Text></Text>
            <Text>Os: <Text strong>{results.coefficients.Os.toFixed(3)}</Text></Text>
          </Space>
        </div>
      </Card>
        ...
        <Card size="small" style={{ borderColor: '#faad14' }}>
          <Space direction="vertical" style={{ width: '100%' }}>
            {warnings.map((warning, idx) => (
              <Text key={idx} type="warning" style={{ fontSize: '13px' }}>
                {warning}
              </Text>
            ))}
          </Space>
...
- Prepare:  10% = ${(results.totalMD * 0.10).toFixed(1)} MD
