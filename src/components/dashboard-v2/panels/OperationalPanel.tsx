    ...
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col span={12}>
          <Statistic
            ...
            prefix={<AlertCircle size={20} style={{ color: metrics.overAllocatedResources.length > 0 ? '#EF4444' : '#10B981' }} />}
            valueStyle={{
        ...
        <Alert
          ...
          description={
            <Space direction="vertical" size="small">
              <Text>
                The following resources have overlapping assignments:
              </Text>
              <Space wrap>
                {metrics.overAllocatedResources.slice(0, 5).map((name, idx) => (
                  <Tag key={idx} color="red">{name}</Tag>
                ))}
      ...
      <div>
        ...
        <Text type="secondary" style={{ fontSize: '12px', marginBottom: '16px', display: 'block' }}>
          Color coding: 🟢 Optimal (0-5 days) | 🟡 Full (6 days) | 🔴 Over-allocated (7+ days) | ⚪ Bench (0 days)
        </Text>
