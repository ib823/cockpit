  ...
  persist(
    (set, get) => ({
      ...
      getSkillGaps: () => {
        ...
        return Array.from(allRequiredSkills).filter(
          skill => !allAvailableSkills.has(skill)
        );
      },
      ...
      removeResource: (resourceId) => {
        set((state) => ({
          ...
          tasks: state.tasks.map(t =>
            t.assignedResource === resourceId
              ? { ...t, assignedResource: null }
              : t
          ),
        }));
      },
      ...
      runOptimization: () => {
        ...
        try {
          ...
          if (offshoreCandidates.length > 3) {
            const totalEffort = offshoreCandidates.reduce((sum, t) =>
              sum + (t.baseEffort * (t.delta / 100)), 0
            );
          ...
          const avgUtilization = updatedResources.length > 0
            ? updatedResources.reduce((sum, r) => sum + r.utilization, 0) / updatedResources.length
            : 0;
