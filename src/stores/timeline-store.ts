...
interface TimelineState {
  ...
  addPhase: (phaseData: Omit<Phase, 'projectId' | 'id' | 'startBusinessDay'> & { id?: string; startBusinessDay?: number }) => void;
  ...
  updatePhaseResourceAllocation: (phaseId: string, resourceId: string, updates: Partial<PhaseResource>) => void;
  addPhaseResource: (phaseId: string, role: ResourceRole, region: string, allocation: number) => void;
    ...
    updatePhaseResources: (phaseId, resources) => {
        set(state => {
            ...
            if (phase) {
                ...
                const validResources = resources.filter((r: Resource) =>
